...
Run the suites with `pnpm eval:tool-routing`, `pnpm eval:progress`, `pnpm eval:context-pressure`, `pnpm eval:long-horizon`, and `pnpm eval:fault-injection`; reports are written to the matching `.eval/*.json` paths. Long-Horizon is a deterministic synthetic coding workflow over disposable local repositories. It compares baseline and managed Harness settings using a Mock LLM; it is not SWE-bench, HumanEval, or a real-model coding benchmark. The Context Pressure suite compares full history, a constrained no-compaction baseline, and deterministic compaction under a 1,900-token context window with 200 reserved output tokens. Fault Injection uses deterministic in-process failures to evaluate provider errors, Tool exceptions and timeouts, invalid and unknown Tool calls, parallel cancellation, context overflow, and committed-call recovery. It is not production chaos testing and does not cover process crashes, MCP disconnects, or real network/provider failures. These offline mock evaluations test Harness policy and runtime behavior; they are not real-model leaderboards or production coding benchmarks.
